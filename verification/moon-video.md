...
## 2026-09-23 Moon 图片与视频输入独立计费选项

本轮 P1，基线为 `main@66784e8ca`、Moon 插件 `1.4.0`。按用户要求，在 Moon `1.5.0` 的所有模型用量 schema 增加 `image_input_count`（张）和 `video_input_count`（段）两个**可选输入单价**。不访问远程 URL 或读取文件内容，只对已通过原有模型限制校验的请求引用条目计数：图片首尾帧、Wan `file_id`、Seedance `content`、PT `materials`/引用数组、底价别名、H3 单数/复数输入、Grok 图片、Responses 图片以及 Canvas 规范化后的素材均走同一计数。重复引用按传入条目计数，不去重；文档/网页和音频不计入这两个字段。无对应素材时返回 0。

原有 `tokens`、`seconds`、`video_count`、`resolution`、`video_input` 不改名、不移除。默认不会自动加价：管理员在 Moon 模型/供应商定价表达式里显式加入新字段才收费，例如按秒模型的示例（美元单位，**未写入实际价格**）：

```text
tier("base", u("seconds") * 0.02 + u("image_input_count") * 0.10 + u("video_input_count") * 0.30)
```

5 秒、2 张图片、2 段视频时，此示例为 0.9 美元；只配置 `u("seconds")` 的旧表达式仍只按输出计价。Token 模型完成时只以真实 `usage.total_tokens` 覆盖预估 Token，图片/视频文件数继续沿用提交时冻结事实；Wan 的 `seconds` 仍包含参考视频时长。**如果给两者同时设非零单价，可能构成额外/重复收费**，应由管理员按业务意图定价并先用报价/测试环境核对。Moon 上游实际报价与本站自定义加价相互独立。
