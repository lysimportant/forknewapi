...
模型 ID 优先精确匹配。不同插件可以声明仅大小写不同的 ID（例如 `MiniMax-H3` 与 `minimax-h3`），两者分别参与路由及计费。只有大小写折叠后指向唯一声明时，宿主才自动纠正输入拼写；有歧义的其他写法不会自动选择插件。此规则不新增 manifest 字段。
...
`meta.modelDiscovery` 为可选的管理功能，声明渠道的只读模型目录。例如：

```js
modelDiscovery: {protocol: "openai", path: "/v1/models"}
```

支持 `openai`（Bearer，`data[].id`）、`gemini`（Google API Key，`models[].name` 与 `nextPageToken`）及 `bailian`（Bearer，`output.models[].model`、`output.total` 与 `page_no/page_size`）协议。`path` 必须是无主机、查询参数、片段、转义或 `.`/`..` 路径段的本地绝对路径，最长 512 字符；宿主按渠道 Base URL 构建请求，避免重复协议前缀。无需在插件中编写 HTTP 请求或解析钩子。

任务插件渠道新增和编辑页面均提供模型获取。声明目录的插件读取上游后，将精确匹配 `meta.models` 的模型提供给管理员勾选；上游返回的其他模型单列为尚未适配，不自动导入或扩大生成路由、能力和计费合同。已有渠道别名与选择保留，由管理员决定是否移除。模型名或目录报价本身不构成生成权限、功能或价格验证。

未声明目录的插件继续使用 `meta.models`，页面明确显示“填入插件模型”及来源，不发起网络请求，也不验证账号权限。上游目录请求失败时明确返回失败，不能静默替换为插件声明列表。`fetchMode` 仍是任务轮询方式，与模型发现无关。

该字段仍属 API v1 的附加能力。旧插件无需修改即可使用静态模型入口；旧宿主会拒绝带此字段的插件，应先升级宿主。新增或维护插件时应确认供应商的真实目录合同、分页与鉴权，不应仅因为生成接口兼容 OpenAI 就假定存在模型目录。
...
提交接口的所有 `2xx` 响应均交给 `parseSubmitResponse` 校验，包含异步受理的 `202`；仅收到 `2xx` 不代表生成完成。

供应商不允许自动重试未知提交结果时，插件在 `requiredCapabilities` 中声明
`"task-submit-no-retry@1"`，并在提交描述中返回 `noRetry: true`。宿主在发送开始后，
对网络错误、非成功状态码、读取或解析失败均禁止自动重发及切换渠道；发送前的校验和价格检查保持原行为。
使用该选项时提交必须包含正文；省略 `body` 或空字符串等明确空正文会在发送前被拒绝，
因为 Go 传输层可以直接重放无正文请求。正常 JSON 和 multipart 正文不受此限制。
省略或设置 `false` 保留已有重试策略。该字段仅作用于提交，不影响只读轮询。
不支持此 capability 的旧宿主会拒绝加载插件，应先升级宿主。幂等请求头仍由插件按照供应商协议构造；
该保护不等于跨 HTTP 请求幂等，客户端收到未知提交结果后必须核对原任务，不能直接重新生成。
