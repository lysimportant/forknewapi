...
export type HostCapability = "json-clone@1" | "submit-sse-delta@1" | "task-submit-no-retry@1";
...
/** 管理页的只读模型目录；path 不含主机、查询参数或凭据，分页由宿主处理。 */
export interface ModelDiscovery {protocol: "openai" | "gemini" | "bailian"; path: string}
export interface Meta {requiredCapabilities?: readonly HostCapability[]; submitResponseTypes?: readonly ("json" | "sse")[]; sortPriority?: number; website?: string; apiVersion: 1; key: string; name: string; icon?: string; description?: LocalizedText; version: string; author: {name: string; url?: string}; baseUrl?: string; channelTypes?: readonly number[]; models: readonly string[]; modelDiscovery?: ModelDiscovery; fetchMode: "per_task" | "batch"; allowedHosts?: readonly string[]; routes?: readonly NativeRoute[]; protocols?: readonly ProtocolClaim[]; usageSchema?: Readonly<Record<string, UsageFieldSchema>>; usageExamples?: readonly UsageExample[]; usageProfiles?: readonly UsageProfile[]; auth?: "none" | "api_key" | "vertex_oauth" | {type: "none" | "api_key" | "oauth2_jwt"}}
