...
## main 回补与服务器更新（2026-09-08）

- 从干净的 main=0e613b56f 回补本次颜色修复，保留主分支的 Responses 修复；不合并功能分支上其他后端、依赖和数据库差异。
- format.ts 的上下文冲突已按主分支结构解决，仅修改延迟分档，不引入无关 reasoning 函数。
- main 无 Vitest 测试配置，测试改用已有 Bun、node:test 与 react-dom/server 静态渲染，无 manifest 或锁文件改动。
- 命令（web 目录）：bun test src/features/usage-logs/components/__tests__/timing-metrics-cell.test.tsx，38 通过；bun run build 通过；5 个改动 TS/TSX 文件的 oxlint、oxfmt 检查通过。
- bun run typecheck 有 31 个 system-settings 既有错误。使用独立的原始 main=0e613b56f worktree 和相同依赖重新运行基线，完整输出逐行一致，确认无新增类型错误；不宣称全量类型检查通过。
- 只修改 6 个前端/测试/文档文件，后端与 Responses 文件无差异。该回补为局部展示修复，无迁移和新增依赖。
- 推送 main 后，服务器仍在 main 且无本地修改时可 git pull --ff-only origin main，再使用原有 Docker 构建/重建命令部署。未操作生产服务器；回滚可使用原镜像或撤销本回补提交后重建，保留数据卷。

## rc.35 兼容升级

以上为历次验证记录。本次 rc.35 保留原分档与渐变条，91 文件/729 项前端测试、类型检查和浅深色浏览器交互通过；详见 [升级记录](../verification/rc35/upgrade-rc35.md)。
