...
export function taskEnumLabel(
  ...
  const label = taskPriceLabel(definition?.enumLabels?.[value], value, language)
  if (value === 'unspecified' && label === value) {
    return t('Provider default (not specified)', { lng: language })
  }
  return label
}
