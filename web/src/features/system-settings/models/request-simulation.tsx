...
import { REASONING_EFFORT_VALUES } from '@/features/pricing/lib/billing-expr'
...
export function RequestSimulation(props: RequestSimulationProps) {
  ...
  const usesEffort =
    compiled.status === 'ready' && compiled.variables.has('effort')
  ...
  return (
    ...
    >
      ...
      <CollapsibleContent className='mt-3 space-y-4'>
        {usesEffort && (
          <Field>
            <FieldLabel htmlFor={effortId}>{t('Reasoning Effort')}</FieldLabel>
            <Select
              value={effort || '__unspecified__'}
              onValueChange={(value) => {
                if (value !== null) {
                  setEffort(value === '__unspecified__' ? '' : value)
                }
              }}
            >
              <SelectTrigger id={effortId}>
                <SelectValue>{effort || t('Default')}</SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value='__unspecified__'>{t('Default')}</SelectItem>
