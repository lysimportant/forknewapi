...
// newAuditTestDatabase 为测试创建隔离数据库，并在测试结束后关闭连接；SQLite 连接必须先于临时目录清理释放。
// 外部数据库仅允许回环地址，每例创建新库，不删除环境变量指定的数据库或表；配置或连接失败时终止测试。
func newAuditTestDatabase(t *testing.T, kind, dsn string) (*gorm.DB, string) {
	...
	if kind == "sqlite" {
		...
		require.NoError(t, err)
		// Cleanup 后注册先执行，确保 Windows 删除 audit.db 前释放主库和独立审计库的文件句柄。
			...
			for _, upgrade := range []bool{false, true} {
				t.Run(fmt.Sprintf("upgrade=%v", upgrade), func(t *testing.T) {
					...
					for i := 0; i < 2; i++ {
						...
						// 每次模拟启动都会替换全局连接，逐次捕获并关闭，不能只释放工厂创建的初始连接。
