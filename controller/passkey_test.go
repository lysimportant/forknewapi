...
func passkeyDomainRequest(t *testing.T, path string, payload any, identity service.AuthIdentity, origin string, handler gin.HandlerFunc) *httptest.ResponseRecorder {
	...
	// 域名用例沿用真实登录契约；缺失或过期协议的拒绝由专门用例覆盖。
	if path == "/api/user/passkey/login/begin" {
		...
		request["consent_version"] = system_setting.CurrentLegalConsentVersion
...
// TestPasskeyDomainLoginRequiresConsent 验证域名选择不能绕过登录协议或创建未授权流程。
func TestPasskeyDomainLoginRequiresConsent(t *testing.T) {
	user, _ := setupSecurityEnrollmentTest(t)
	for _, consent := range []struct {
		name, version, code string
		agreed              bool
	}{
		{"missing", "", "legal_consent_required", false},
		{"unchecked", system_setting.CurrentLegalConsentVersion, "legal_consent_required", false},
		{"outdated", "old-version", "legal_consent_outdated", true},
	} {
		t.Run(consent.name, func(t *testing.T) {
			body, err := common.Marshal(passkeyLoginBeginRequest{RPID: "example.com", Consent: consent.agreed, ConsentVersion: consent.version})
			require.NoError(t, err)
			response := securityEnrollmentRequest(http.MethodPost, "/api/user/passkey/login/begin", string(body), "", service.AuthIdentity{}, PasskeyLoginBegin)
			var result securityEnrollmentResponse
			require.NoError(t, common.Unmarshal(response.Body.Bytes(), &result))
			assert.False(t, result.Success)
			assert.Equal(t, consent.code, result.Code)
			assert.Empty(t, response.Header().Values("Set-Cookie"))
			var flows int64
			require.NoError(t, model.DB.Model(&model.AuthFlow{}).Where("purpose = ?", model.AuthFlowPurposePasskeyLogin).Count(&flows).Error)
			assert.Zero(t, flows)
			sessions, err := model.CountActiveUserSessions(user.Id, time.Now().Unix())
			require.NoError(t, err)
...
func TestPasskeyDomainsRejectInvalidAssertions(t *testing.T) {
	for _, failure := range []string{"wrong RP ID", "unlisted origin", "origin outside RP scope", "wrong user", "bad signature", "missing user verification", "expired challenge", "removed domain", "known different domain", "missing consent version", "outdated consent version"} {
		t.Run(failure, func(t *testing.T) {
			...
			case "missing consent version", "outdated consent version":
				flow, err := model.GetAuthFlow(begin.FlowToken, model.AuthFlowMatch{Purpose: model.AuthFlowPurposePasskeyLogin})
				require.NoError(t, err)
				var payload struct {
					SessionData webauthn.SessionData    `json:"session_data"`
				...
				require.NoError(t, common.UnmarshalJsonStr(flow.Payload, &payload))
				payload.Security.ConsentVersion = ""
				if failure == "outdated consent version" {
					payload.Security.ConsentVersion = "old-version"
				}
				encoded, err := common.Marshal(payload)
				require.NoError(t, err)
				require.NoError(t, model.DB.Model(flow).Update("payload", string(encoded)).Error)
	...
	for _, locale := range []struct {
		...
		t.Run(locale.language, func(t *testing.T) {
			for _, request := range []struct {
				...
				body := request.body
				if request.path == "/api/user/passkey/login/begin" {
					encoded, err := common.Marshal(passkeyLoginBeginRequest{RPID: "unconfigured.example.com", Consent: true, ConsentVersion: system_setting.CurrentLegalConsentVersion})
					require.NoError(t, err)
					body = string(encoded)
				}
				c.Request = httptest.NewRequest(method, request.path, strings.NewReader(body))
