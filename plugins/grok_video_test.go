...
func TestGrokVideoContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "提交响应保留上游ID", hook: "parseSubmitResponse", args: []any{nil, map[string]any{"body": map[string]any{"id": "vendor-id", "status": "queued"}}}, want: `{"taskId":"vendor-id","taskData":{"id":"vendor-id","status":"queued"}}`},
