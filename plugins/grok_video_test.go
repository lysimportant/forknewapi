...
func TestGrokVideoContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "multipart文件引用原样传宿主", hook: "buildSubmitRequest", args: []any{map[string]any{"baseUrl": "https://upstream.example/v1", "apiKey": "fixture-only", "upstreamModel": "grok-imagine-video-1.5", "requestBody": map[string]any{"prompt": "ocean"}, "files": []any{map[string]any{"field": "input_reference", "ref": "opaque-1", "filename": "frame.png"}}}}, want: `{"url":"https://upstream.example/v1/videos/generations","method":"POST","headers":{"Authorization":"Bearer fixture-only"},"bodyType":"multipart","parts":[{"name":"prompt","value":"ocean"},{"name":"model","value":"grok-imagine-video-1.5"},{"name":"input_reference","fileRef":"opaque-1","filename":"frame.png"}]}`},
	}
