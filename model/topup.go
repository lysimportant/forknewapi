...
// creditTopUpQuota 在订单事务内增加到账额度并发放邀请返佣；sourceKey 为唯一充值来源。
// 条件更新原子限制余额上限。充值或返佣失败返回错误，由调用方回滚整个事务。
func creditTopUpQuota(tx *gorm.DB, userId int, creditedQuota int, updates map[string]any, sourceKey string) error {
	maxCurrentQuota, err := topUpQuotaMaxCurrent(creditedQuota)
	if err != nil {
		return err
	}
	...
	if result.RowsAffected == 1 {
		return grantTopUpInviteReward(tx, userId, sourceKey, creditedQuota)
	}
...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		return creditTopUpQuota(tx, topUp.UserId, quotaToAdd, nil, fmt.Sprintf("topup:%d", topUp.Id))
	})
	if err != nil {
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 增加用户额度（立即写库，保持一致性）
		if err := creditTopUpQuota(tx, topUp.UserId, quotaToAdd, nil, fmt.Sprintf("topup:%d", topUp.Id)); err != nil {
			return err
		}
...
func RechargeCreem(referenceId string, customerEmail string, customerName string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		return creditTopUpQuota(tx, topUp.UserId, quota, updateFields, fmt.Sprintf("topup:%d", topUp.Id))
	})
