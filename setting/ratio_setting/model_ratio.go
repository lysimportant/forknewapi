...
func GetCompletionRatio(name string) float64 {
	...
	// User-configured completion ratios always take priority over hardcoded defaults.
	// Hardcoded values are defaults only and never lock pricing.
...
func GetCompletionRatioInfo(name string) CompletionRatioInfo {
	...
	// Never report locked: admins may freely override any completion ratio / price.
	if ratio, ok := completionRatioMap.Get(name); ok {
