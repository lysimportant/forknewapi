	...
	attempts:
		for attempt := 0; ; attempt++ {
			...
			if attempt == 0 {
				...
				if !expectRetry {
					// 先半关闭写方向并读完客户端剩余控制帧，避免 Windows 将带未读数据的 Close 转为 TCP RST。
			...
			res.err = writeH2TestResponse(framer, streamID)
			if res.err == nil {
	...
	go func() {
		...
		for attempt := 0; attempt < 2; attempt++ {
			...
			if attempt == 0 {
				...
				if err == nil {
					// 确保客户端看到 GOAWAY 和 FIN，而不是关闭时未读 SETTINGS ACK 引起的 TCP RST。
			...
			// 重试成功的连接也必须优雅关闭，否则响应可能被 TCP RST 抢先中断。
			if err == nil {
				err = conn.(*net.TCPConn).CloseWrite()
			}
			if err == nil {
