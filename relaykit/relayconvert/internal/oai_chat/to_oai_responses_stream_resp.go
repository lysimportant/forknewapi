...
type ChatToResponsesStreamState struct {
	// AllowCumulativeToolNames 允许宿主传入严格前缀扩展的累积名称；默认保留原生 Chat 更名校验。
	AllowCumulativeToolNames bool
	ID                       string
	Model                    string
	Created                  int64
	Usage                    *dto.Usage
...
func (s *ChatToResponsesStreamState) appendToolCallDelta(toolCall dto.ToolCallResponse) ([]ChatToResponsesStreamEvent, error) {
	...
	if incomingName != "" {
		if tool.Name != "" && tool.Name != incomingName && !(s.AllowCumulativeToolNames && strings.HasPrefix(incomingName, tool.Name)) {
