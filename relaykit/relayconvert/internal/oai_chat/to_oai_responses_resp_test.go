...
// TestResponsesCumulativeToolNameOptIn 验证名称片段兼容必须显式启用，且始终拒绝非前缀更名，保护 rc35 默认严格语义。
func TestResponsesCumulativeToolNameOptIn(t *testing.T) {
	for _, test := range []struct {
		...
		{"默认拒绝更名", false, "read_file", true},
		{"显式允许累积片段", true, "read_file", false},
		{"启用后仍拒绝替换", true, "write_file", true},
	} {
		t.Run(test.name, func(t *testing.T) {
			state := NewChatToResponsesStreamState("resp_tools", "fixture")
			state.AllowCumulativeToolNames = test.enabled
			for i, name := range []string{"read_", test.next} {
				_, err := ChatCompletionsStreamChunkToResponsesEvents(&dto.ChatCompletionsStreamResponse{
					Choices: []dto.ChatCompletionsStreamResponseChoice{{Index: 0, Delta: dto.ChatCompletionsStreamResponseChoiceDelta{
						ToolCalls: []dto.ToolCallResponse{{Index: lo.ToPtr(0), ID: "call_1", Type: "function", Function: dto.FunctionResponse{Name: name}}},
					}}},
				}, state)
				if i == 1 && test.wantError {
					require.Error(t, err)
					assert.Contains(t, err.Error(), "changed name")
				} else {
					require.NoError(t, err)
				}
			}
