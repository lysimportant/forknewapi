...
// TestFrontendSimulationContract 用共享样本验证前端模拟器与实际计费引擎的结果一致。
func TestFrontendSimulationContract(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			require.NoError(t, err)
			// 样本使用表达式变量名，img_cr/img_o 不能按 Go 字段名自动反序列化。
			tokens := billingexpr.TokenParams{
				P: tc.Tokens["p"], C: tc.Tokens["c"], Len: tc.Tokens["len"],
				CR: tc.Tokens["cr"], CC: tc.Tokens["cc"], CC1h: tc.Tokens["cc1h"],
				Img: tc.Tokens["img"], ImgCR: tc.Tokens["img_cr"], ImgO: tc.Tokens["img_o"],
				AI: tc.Tokens["ai"], AO: tc.Tokens["ao"],
			}
			cost, trace, err := billingexpr.RunExprWithRequest(tc.Expression, tokens, billingexpr.RequestInput{Body: body, Headers: tc.Headers, Usage: tc.Usage, ImageCount: tc.ImageCount, Effort: tc.Effort})
			require.NoError(t, err)
